...
class Config:
    ...
    # Giga-Path model parameters
    model_name = "hf_hub:prov-gigapath/prov-gigapath"
    feature_dim = 1536  # Giga-Path output dimension (needs to be verified)
...
def setup_logging(output_dir):
    ...
    """
    log_file = os.path.join(output_dir, "giga_path_feature_tsne.log")
    ...
    logger.info("=" * 60)
    logger.info("Giga-Path Feature Extraction and t-SNE Visualization Started")
    logger.info("Model: Giga-Path (from Hugging Face timm)")
    logger.info("=" * 60)
...
def initialize_giga_path_model(config, device, logger=None):
    ...
    """
    if logger:
        logger.info("Initializing Giga-Path model...")
    
    print("Initializing Giga-Path model...")
    print("Model: Giga-Path (from Hugging Face timm)")
    
    try:
        # Load Giga-Path model using timm
        ...
        if logger:
            logger.info("Giga-Path model initialized successfully")
            logger.info(f"Feature dimension: {config.feature_dim}")
        
        print("Giga-Path model initialized successfully")
        ...
        error_msg = f"Failed to initialize Giga-Path model: {e}"
        print(error_msg)
        if logger:
            logger.error(error_msg)
        raise e

def extract_features_with_giga_path(model, preprocess, image, device):
    """Extract features using Giga-Path model
    
    Args:
        model: Giga-Path model
...
def extract_features_from_patches(config, filtered_patches_df, output_dir, logger=None):
    ...
    """
    print("\n" + "="*60)
    print("Feature Extraction with Giga-Path Model")
    print("="*60)
    
    if logger:
        logger.info("Feature Extraction with Giga-Path Model")
        logger.info("="*60)
    ...
    model, preprocess = initialize_giga_path_model(config, device, logger)
    
    # Create features directory with model name
    features_dir = os.path.join(output_dir, "features_GigaPath")
    os.makedirs(features_dir, exist_ok=True)
    
    # Group patches by WSI
    wsi_groups = filtered_patches_df.groupby('wsi_id')
    
    desc = "Extracting Giga-Path features"
    ...
    for wsi_id, wsi_patches in tqdm(wsi_groups, desc=desc):
        ...
        for idx, row in wsi_patches.iterrows():
            ...
            try:
                ...
                # Extract features
                features = extract_features_with_giga_path(model, preprocess, patch, device)
...
def main():
    ...
    print("Giga-Path Feature Extraction and t-SNE Visualization Pipeline Starting")
    print("="*60)
    print(f"Input directory: {args.input_dir}")
    print("Model info: Giga-Path (from Hugging Face timm)")
